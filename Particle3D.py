...
class Particle3D(object):
    ...
    def leapPos2nd(self, dt, force, boxDim):
    ...
    def vectorSeparation(p1, p2, boxDim):
        ...
        for i in range(3):
            ...
            separation[i] = separation[i] - int(separation[i]/(boxDim[i]/2.0))*boxDim[i]
        return separation
    ...
    def particleForce(p1, p2, rc, boxDim):
        if (Particle3D.scalarSeparation(p1, p2, boxDim)) > rc:
            return 0.
        else:
            if (Particle3D.scalarSeparation(p1, p2, boxDim)) == 0:
                return 0.
            else:
                return (48*(1./(Particle3D.scalarSeparation(p1, p2, boxDim)**14)-1./((Particle3D.scalarSeparation(p1, p2, boxDim)**8)*2.)))*(Particle3D.vectorSeparation(p1, p2, boxDim))
    ...
    def particlePotEnergy(p1, p2, rc, boxDim):
        if (Particle3D.scalarSeparation(p1, p2, boxDim)) > rc:
            ...
            if (Particle3D.scalarSeparation(p1, p2, boxDim)) == 0:
                ...
                return 4.*((1./Particle3D.scalarSeparation(p1, p2, boxDim)**12)-(1./Particle3D.scalarSeparation(p1, p2, boxDim)**6))
    ...
    @staticmethod
    def positionUpdateList(pList, force, dt, boxDim):
        for i, el in enumerate(pList):
            el.leapPos2nd(dt, force[i], boxDim)
