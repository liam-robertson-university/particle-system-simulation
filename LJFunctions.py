...
def checkPBC(boxDim, particleList):
    ...
    for p in particleList:
        ...
        for i in range(3):
            # Check if particle has gone out the boundary on the the ith (x,y,z) axis and move it to its correct position
            if p.position[i] < 0:
                while p.position[i] < 0:
                    p.position[i] += boxDim[i]
            if p.position[i] >= boxDim[i]:
                while p.position[i] >= boxDim[i]:
                    p.position[i] -= boxDim[i]
